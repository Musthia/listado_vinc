...
from clientd.gui_app_uba import Frame, barra_menu
from datetime import datetime
from clientd.gui_app_uba import Frame   
from clientd.gui_app_uba import abrir_whatsapp
...
class AplicacionPrincipal(tk.Tk):
    def __init__(self, nivel_de_seguridad, nombre_usuario, rol_usuario):
        ...
        self.title(f"Base de Datos: IPS - Usuario: {nombre_usuario} | Rol: {rol_usuario}")
...
#
        ...
        title_label = tk.Label(self, text="DATCORR: Archivo: ✅ IPS", font=font_title, fg="#5c5c5c")
#
...
class InicioSesionVentana(tk.Tk):
    ...
    def verificar_credenciales(self):
        ...
        if usuario == usuario_maestro and contrasena == contrasena_maestra:
            ...
            messagebox.showinfo("ATENCION", "¡ REALIZAR COPIA DE SEGURIDAD !")
            if hasattr(self, 'after_id'):
                self.after_cancel(self.after_id)  # ✅ Cancela el reloj antes de cerrar
        ...
        if resultado:
            ...
            if hasattr(self, 'after_id'):
                self.after_cancel(self.after_id)  # ✅ También aquí         

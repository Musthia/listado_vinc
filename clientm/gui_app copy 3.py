...
#
    ...
    def pegar_portapapeles(self):
        try:
            ...
            zona = self.clipboard_get().strip()

            if zona:  # Verifica que no esté vacío
                # Establecer el contenido en el entry_zona
                self.entry_zona.delete(0, tk.END)
                self.entry_zona.insert(0, zona)

                # Llamar a consultar_dato con el valor del portapapeles
                denominacion = self.consultar_dato(zona)

                if denominacion:
                    # Actualizar el campo entry_denominacion
                    self.entry_denominacion.delete(0, tk.END)
                    self.entry_denominacion.insert(0, denominacion)
                else:
                    # Mostrar mensaje si no hay coincidencia
                    self.mostrar_mensaje_temporal("No se encontró una coincidencia.", 1000)

                # Mover el foco al botón Guardar
                self.boton_guardar.focus_set()
            else:
                self.mostrar_mensaje_temporal("El portapapeles está vacío.", 1000)
        except Exception as e:
            print(f"Error al pegar desde el portapapeles: {e}")
            messagebox.showerror("Error", "Ocurrió un problema al procesar el dato del portapapeles.")
...
#
    ...
    def tabla_Catastro_database(self):
        ...
        #self.button_guardar.bind("<Return>", lambda event: self.guardar_cambios(self.entry_denominacion_resultado, self.entry_zona_resultado, self.entry_lote_resultado,self.entry_caja_resultado,))
    ...
    def mostrar_resultados(self, rows):
        ...
        button_editar.config(width=15, font=("Arial",10, "bold"), fg = "black", bg="#00aae4",cursor = "hand2", activebackground= "#FFFFFF")
    ...
    def editar_datos_consulta(self, id_resultado,
        ...
        label_nombre = tk.Label(ventana_edicion, text="Edicion de DATOS de consulta")
        ...
        self.entry_resgistro_resultado.config(width= 23, font = ("Arial", 10, "bold"))
...
class FiltroFechaVentana(tk.Toplevel):
    def __init__(self, master=None):
        ...
        self.button_filtrar.config(width=15, font=("Arial",10, "bold"), fg = "black", bg="#00aae4",cursor = "hand2", activebackground= "#FFFFFF")
        self.button_filtrar.bind("<Return>", lambda event: self.filtrar)
        #self.button_filtrar.config(width=12, font=("M_karina",15, "bold"), fg = "black", bg="#00aae4",cursor = "hand2", activebackground= "#FFFFFF")
 """
        self.button_filtrar = tk.Button(self, text="FILTRAR", command=self.filtrar)
    ...
    def editar_datos_consulta(self, id_resultado,
        ...
        label_nombre = tk.Label(ventana_edicion, text="Edicion de DATOS filtrado por fecha")
        label_nombre.config(font=("Linkin Park ExtraBold", 15), background= "#35BD6D") 
        label_nombre.grid(row=0, column=0, columnspan=2)  # Se extiende a través de dos columnas
